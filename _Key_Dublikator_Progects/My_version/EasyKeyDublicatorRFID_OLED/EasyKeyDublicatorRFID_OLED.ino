...
#define G_Led 3
#define B_Led 4
#define LedType 1          // тип питания светодиона 1 - общий плюс+, 0-
...
void clearLed() {
  digitalWrite(R_Led, LedType);
  digitalWrite(G_Led, LedType);
  digitalWrite(B_Led, LedType);
}
...
void loop() {
  ...
  if (echo == 'u') { //добавляем универсальные ключи
    Add_Defaukt_keys();
  }

  if (echo == 'w') { //пишем все ключи з памяти в порт
    WriteAllKeyfromEEPROM();
  }

  if (echo == 'c') { //пишем все ключи з памяти в порт в кодовом формате
    WriteAllKeyfromEEPROM_codeFormat();
  }

  if ((echo == 't') || enc1.isClick()) {  // переключаель режима чтение/запись
    switch (copierMode) {
      case md_empty: Sd_ErrorBeep(); break;
      case md_read: copierMode = md_write; clearLed(); digitalWrite(R_Led, !LedType);  break;
      case md_write: copierMode = md_blueMode; clearLed(); digitalWrite(B_Led, !LedType);
        digitalWrite(Luse_Led, !digitalRead(Luse_Led)); break;
      case md_blueMode: copierMode = md_read; clearLed(); digitalWrite(G_Led, !LedType);
        digitalWrite(Luse_Led, !digitalRead(Luse_Led)); break;
    }
  ...
  switch (copierMode) {
    case md_empty: case md_read:
      ...
      break;
    case md_write:
      if (keyType != keyEM_Marine) write2iBtn();
      else
      {
        write2rfid();
        write2iBtn();
