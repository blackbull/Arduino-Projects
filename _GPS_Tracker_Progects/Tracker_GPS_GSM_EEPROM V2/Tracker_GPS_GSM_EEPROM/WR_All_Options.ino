...
String Get_Last_GPS_Data_String(int NeedCount, int SkipCount) { //выводим максимум 16 строк, а то оперативки не хватит

  if (NeedCount > 16) NeedCount = 16;
  ...
  while (lastGPSADRTemp >= GPSstartAdress && countFull < NeedCount) //считываем все старые координаты до старта секции памяти
  {
    if (countBreak < SkipCount) {
      countBreak++;
    ...
    if (countFull != NeedCount) ret += F("\n"); //добавляем ентер после каждой строки, кроме последней

    lastGPSADRTemp -= sizeof(GpsDataStruct);
  }


  lastGPSADRTemp = GPSstartAdress; //стартовый адресс
  while (lastGPSADRTemp < 1023 - (2 * sizeof(GpsDataStruct))) {
    lastGPSADRTemp += sizeof(GpsDataStruct); //ищем последний адресс в памяти
  }

  while (lastGPSADRTemp > LastWritingSTR.LastAdress && countFull < NeedCount) //считываем до последней записи
  {
    if (countBreak < SkipCount) {
      countBreak++;
      lastGPSADRTemp -= sizeof(GpsDataStruct);
