...
bool InitialGSM() {
  ...
  DEBUG_ln_2(F("InitialGSM -> Start"));

  String ans = sendATCommand(F("AT"), true);
  ans.trim();
  if ( ans.endsWith(F("OK")))      // Отправили AT для настройки скорости обмена данными
  {
    //sendATCommand("AT+CMGDA=\"DEL ALL\"", true);               // Удаляем все SMS, чтобы не забивать память

    // Команды настройки модема при каждом запуске
    sendATCommand(F("ATE0"), true);                                //ывключаем ЭХО
    sendATCommand(F("AT+CLIP=1"), true);                           // Включаем АОН
    //_response = sendATCommand("AT+DDET=1", true);             // Включаем DTMF
    // sendATCommand("AT+CPMS=SM,SM,SM", true);                  //установить память для сохранения смс -> сим карта
    sendATCommand(F("AT+CNMI=1,0"), true);                       //не выводить сообщение о пришедших смс, а сразу сохранять их в сим
    sendATCommand(F("AT+CMGF=1;&W"), true);                        // Включаем текстовый режима SMS (Text mode) и сразу сохраняем значение (AT&W)(не сохраняем, что бы не расходовать ресурс памяти)!
    InitialGSMFlag = true;
