...
static byte repeatNEEDCnt = 2; //необходимое количество повторов сигнала, что бы считать корректный прием
...
void OLED_printSnifferMode() {
  switch (snifferMode) {
    case smdNormal: myOLED.clrScr(); myOLED.print(String(repeatNEEDCnt) + " N", RIGHT, 24); break;
    case smdAutoRec: myOLED.clrScr(); myOLED.print(String(repeatNEEDCnt) + " A", RIGHT, 24); break;
...
void handleInt() {
  ...
  if ((duration > minPause) && (duration < maxPause) && (changeCnt >= 24)) { //похоже на очередной синхроимпульс
    ...
    if (abs(delta) < (350 >> prescal)) { // это точно повторный сигнал
      repeatCnt++;
      if (repeatCnt >= repeatNEEDCnt) { //----------------менять чувствительность здесь-------------------------------------------------------------------
        recieved = true;
        logLen = changeCnt;
        repeatCnt = 0;
        changeCnt = 0;
...
void loop() {
  ...
  if (btn_left.isSingle() && (EEPROM_key_count > 0)) {      //при повороте энкодера листаем ключи из eeprom
    EEPROM_key_index--;
  ...
  if (btn_right.isSingle() && (EEPROM_key_count > 0)) {
    EEPROM_key_index++;
    if (EEPROM_key_index > EEPROM_key_count) EEPROM_key_index = 1;
    EEPROM_get_key(EEPROM_key_index, &keyData1);
    OLED_printKey(&keyData1);
  ...
  if (btn_left.isDouble()) {
    if (repeatNEEDCnt == 2)repeatNEEDCnt = 1;
    else if (repeatNEEDCnt == 1)repeatNEEDCnt = 2;
